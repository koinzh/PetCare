...
/* TODO: 替换为自己设备的三元组 */
char *product_key       = "k0txi2jzZL9";
char *device_name       = "raspberry_4b";
char *device_secret     = "6e0f2762dba515a35eb098b138d90405";

/*
    TODO: 替换为自己实例的接入点

    对于企业实例, 或者新版本公共实例(有实例信息一栏, 默认显示实例ID值)
...
*/
char  *mqtt_host = "iot-06z00ecae6zqnin.mqtt.iothub.aliyuncs.com";
 ...
 *
 * 例如: [1577589489.033][LK-0317] mqtt_basic_demo&a13FN5T****
 *
    ...
    switch (recv->type) {
        ...
        case AIOT_DMRECV_PROPERTY_SET: {
            printf("msg_id = %ld, params = %.*s\r\n",
             ...
             char *light_on = "{\"LightSwitch\":1}";
             char *light_off = "{\"LightSwitch\":0}";
             char *result = strstr(recv->data.property_set.params, light_on);
             if(NULL != result) {
                demo_send_property_post(dm_handle, light_on);
             }
             result = strstr(recv->data.property_set.params, light_off);
             if(NULL != result) {
                demo_send_property_post(dm_handle, light_off);
             }
